...
class ConsumerQueries {
  ...
  static String OBTAIN_WORK_QUERY_TRANSACTION_SCOPED =
      "UPDATE queue"
          ...
          + "            LIMIT 64"
          + "        ) akh"
          + "        WHERE pg_try_advisory_xact_lock('queue'::regclass::int, ('x'||substr(md5(\"group\"),1,8))::bit(32)::int)"
          + "        ORDER BY id"
          + "        LIMIT 1"
          + "    )"
          + " AND available"
  ...
  static String OBTAIN_WORK_QUERY_SESSION_SCOPED =
      "UPDATE queue"
          ...
          + "            LIMIT 64"
          + "        ) ss"
          + "        WHERE pg_try_advisory_lock('queue'::regclass::int, ('x'||substr(md5(\"group\"),1,8))::bit(32)::int)"
          + "        ORDER BY id"
          + "        LIMIT 1"
          + "    )"
          + " AND available"
